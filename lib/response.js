...
class ApiResponse {
    ...
    setDefaultCors(acao, acam, acah) {
        this.headers["Access-Control-Allow-Origin"] = acao || http_1.HttpHeadersCors.ORIGIN;
        this.headers["Access-Control-Allow-Methods"] = acam || http_1.HttpHeadersCors.METHODS;
        this.headers["Access-Control-Allow-Headers"] = acah || http_1.HttpHeadersCors.HEADERS;
    }
    getDefaultCors() {
        return [
    ...
    json(body) {
        return this
            .header("content-type", http_1.HttpContentTypes.JSON)
            .send(body);
    }
    html(body) {
        return this
            .header("content-type", http_1.HttpContentTypes.HTML)
            .send(body);
    }
    file(body) {
        return this
            .header("content-type", http_1.HttpContentTypes.BINARY)
            .send(body);
    }
    error(payload) {
        var _a;
        return this.send({
            status: payload.status || http_1.HttpStatus.INTERNAL_SERVER_ERROR,
            code: payload.code,
            message: payload.message,
            ...((_a = payload.data) !== null && _a !== void 0 ? _a : {}),
        }, true);
    }
}
